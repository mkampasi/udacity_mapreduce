...
def mapper():
    ...
    for line in reader:
        if len(line)<5:
            continue
        body = line[4]
        if (body.find('!')!=-1 or body.find('.')!=-1 or body.find('?')!=-1):
            if ((body.find('!')==-1 or body.find('!')==len(body)-1) and 
            (body.find('?')==-1 or body.find('?')==len(body)-1) and
            (body.find('.')==-1 or body.find('.')==len(body)-1) ):
